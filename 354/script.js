...
function getMovies(keyword) {
  return fetch('http://www.omdbapi.com/?apikey=6c4aecc&s=' + keyword)
    ...
    .then(response => {
      //console.log(response);//we can use the data inside this
      //the first response below is from the promise (response above), while the 2nd Response below is from json key
      if (response.Response === 'False') {
        throw new Error(response.Error); //we display the error message from the console.log above
      }
      //console.log(response); //try to type a movie then search. There is Search. We take the Search: Array(10) if the Response: "True". Now try to uncomment the updateUI(movies); above. Go to 355
