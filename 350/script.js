...
//when we simply click the search button or type unavailable film there will be none to be displayed in the console as there's no error being caught at above: catch(err), as fetch will only reject if the network is in error for example we remove the p in http
//we also want to capture the error when the film is unavailable. We need to handle it manually in the response.json() below
//if the API key is wrong, the unauthorized error will be displayed in console. The fetch is working, but unauthorized.
//how to capture the error? we do it manually. Go to 351
